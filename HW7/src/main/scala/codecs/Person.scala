...
import codecs.JsonReader.{getField, getOptionField, objectReader}
...
object Person {
  ...
  implicit def managerReader: JsonReader[Manager] = objectReader(map =>
    for {
      ...
      boss <- getOptionField[Manager](map, "boss")(managerReader)
  ...
  implicit def emploeeWriter: JsonWriter[Employee] =
    employee =>
      JsonObject(
        Map(
